...
execute_seek(gint64 desired_position, CustomData *data) {
    ...
    if (GST_CLOCK_TIME_IS_VALID (data->last_seek_time) &&
        diff < SEEK_MIN_DELAY) {
...
buffering_cb(GstBus *bus, GstMessage *msg, CustomData *data) {
    ...
    GST_DEBUG ("target_state %d percentage %d", data->target_state, percent);
...
check_initialization_complete(CustomData *data) {
    ...
    if (!data->initialized && data->native_window && data->main_loop) {
        ...
        GstElement *video_sink = gst_bin_get_by_interface(
                GST_BIN(data->pipeline), GST_TYPE_VIDEO_OVERLAY);
        if (video_sink) {
...
gst_native_set_uri(JNIEnv *env, jobject thiz, jstring uri) {
    ...
    GstStateChangeReturn r = gst_element_set_state(data->pipeline,
                                                   GST_STATE_PLAYING);
    GST_DEBUG ("gst_native_set_uri gst_element_set_state returned: %d", r);
    data->is_live =
...
gst_native_play(JNIEnv *env, jobject thiz) {
    ...
    GstStateChangeReturn r = gst_element_set_state(data->pipeline,
                                                   GST_STATE_PLAYING);
    GST_DEBUG ("gst_element_set_state returned: %d", r);
...
gst_native_class_init(JNIEnv *env, jclass klass) {
    ...
    set_message_method_id =
            (*env)->GetMethodID(env, klass, "setMessage",
                                "(Ljava/lang/String;)V");
...
JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    if ((*vm)->GetEnv(vm, (void **) &env, JNI_VERSION_1_4) != JNI_OK) {
        __android_log_print(ANDROID_LOG_ERROR, "kaonic-streamer",
                            "Could not retrieve JNIEnv");
        return 0;
    }
    jclass klass = (*env)->FindClass(env,
                                     "network/beechat/kaonic/video/VideoStreamingActivity");
